...
class LocalJobFriendlyTerraformCommandFactorySpec extends Specification implements TestDefaults {
  ...
  void "terraformCommand handles baseCommand as null, empty string and real string"() {
    ...
    when:
      def packerCommand = commandFactory.buildTerraformCommand(baseCommand, parameterMap, null, "")
  ...
  void "packerCommand includes -varFileName only when 'varFile' is specified; varFile is #varFile"() {
    ...
    when:
      def packerCommand = commandFactory.buildTerraformCommand("", parameterMap, varFile, "")
  ...
  void "packerCommand includes parameter with non-quoted string"() {
    ...
    when:
    def packerCommand = commandFactory.buildTerraformCommand("", parameterMap, null, "")
