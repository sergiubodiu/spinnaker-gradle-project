...
class JobOperationsController {
	...
	public void getLogFile(HttpServletResponse response, @PathVariable long executionId)
		...
		File downloadFile = new File(loggingPath + getBaseName(jobExecution));
  ...
  private String getBaseName(JobExecution jobExecution) {
    return "batch-" + jobExecution.getJobInstance().getJobName() + "-" + jobExecution.getId() + ".log";
  }
